...
const AssignWorkflow = () => {
    ...
    const [selectedWorkflow, setSelectedWorkflow] = useState<{ WORKFLOW_ID: string } | null>(null);
    ...
    const selectedCountryTemplate = (option: { WORKFLOW_NAME: string }, props: DropdownProps) => {
        if (option) {
            return (
    ...
    const countryOptionTemplate = (option: { WORKFLOW_NAME: string }) => {
        return (
            <div className="flex align-items-center">
                <div>{option.WORKFLOW_NAME}</div>
            </div>
